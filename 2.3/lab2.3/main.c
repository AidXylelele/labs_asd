...
void set_side_positions(node_pos_t *positions, int node_count, int *index, node_pos_t(*get_pos)(int, double)) {
    int spaced = 0;
    double k = 0;
    for (int i = 0; i < node_count; i++) {
        k += 2*M_PI/11;
        node_pos_t pos = get_pos(spaced, k);
        positions[*index] = pos;
        *index += 1;
        spaced++;
    }
...
node_pos_t get_top_position(int spaced, double t) {
    node_pos_t pos;
    pos.x =  (window_width / 2) + 2*NODE_SPACING*cos(t);
    pos.y =  (window_height / 2) + 2*NODE_SPACING*sin(t);
    return pos;
}

node_pos_t get_center_position(int spaced, double t) {
    node_pos_t pos;
...
node_pos_t *get_node_positions() {
    ...
    int index = 0;
    set_side_positions(positions, window_field.all, &index, get_top_position);
    set_side_positions(positions, window_field.center, &index, get_center_position);
    return positions;
}
...
void draw_node(cairo_t *cr, node_pos_t pos, char *text) {
    ...
    cairo_arc(cr, pos.x, pos.y, NODE_RADIUS, 0, 2 * M_PI);
    cairo_stroke_preserve(cr);
...
void calculate_size() {
    ...
    window_field.center = 1;
    window_field.all = NODE_COUNT - window_field.center;
...
void create_application(int argc, char *argv[]) {
    GtkApplication *app = gtk_application_new("o.g_vlad", G_APPLICATION_FLAGS_NONE);
    g_signal_connect(app, "activate", G_CALLBACK(on_app_activate), NULL);
    g_application_run(G_APPLICATION(app), argc, argv);
}

void directed_read() {
    printf("Choose type of your graph? (0 for undirected, any other number for directed)\n");
    scanf("%d", &directed);
...
void node_read() {
    ...
    while (node_shown < -1 || node_shown >= NODE_COUNT) {
        printf("What node connections to show? (press -1 to show all node connections)\n");
        scanf("%d", &node_shown);
    }
}
